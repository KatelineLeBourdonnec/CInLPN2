...
/*
 individual contribution to the MatCov estimand
 */

//[[Rcpp::export]]
arma::mat MatCov(int K, int nD, arma::mat matrixP, int m_i, arma::vec tau, arma::vec tau_i, arma::mat Ytildi, 
                 arma::mat x0i, arma::mat z0i, arma::mat xi, arma::mat zi, arma::colvec alpha_mu0,
                 arma::colvec alpha_mu, arma::mat matDw,  arma::mat matDw_u, arma::mat matDu,
                 arma::mat matB, arma::mat Sig, arma::mat G_mat_A_0_to_tau_i,
                 arma::mat G_mat_prod_A_0_to_tau,  double DeltaT){
  ...
  // ###### compute  Yi - E(Yi) ##### deleting missing values #####
  vec nu_i =  - YiNui(nD, matrixP, tau, tau_i, DeltaT, Ytildi, x0i, alpha_mu0, xi, alpha_mu, G_mat_A_0_to_tau_i);
  vec Ytildi_nu_i = YiNui(nD, matrixP, tau, tau_i, DeltaT, Ytildi, x0i, alpha_mu0, xi, alpha_mu, G_mat_A_0_to_tau_i);
  
  //  ##### computing of the  matrix  matVY_i #####
  // int sizeYi = nu_i.size();
  
  int sizeYi = Ytildi_nu_i.size();
  mat matVY_i = zeros<mat>(sizeYi,sizeYi); // initialization of the  matVY_i to zero
  vec k_i = zeros<vec>(m_i);// vector of number of observed marker at each observation time
  int q = zi.n_cols; // number of random effets on the slope
  mat resVC = zeros<mat>(sizeYi,sizeYi); // initialization of the  matVY_i to zero
  ...
  resVC = matVY_i;
  //resVC(span(1,sizeYi),span(1,sizeYi)) = matVY_i;
  // resVC(sizeYi+1,span(1,sizeYi)) = nu_i;
...
//[[Rcpp::export]]
arma::vec Esperance(int K, int nD, arma::mat matrixP, int m_i, arma::vec tau, arma::vec tau_i, arma::mat Ytildi, 
                    arma::mat x0i, arma::mat z0i, arma::mat xi, arma::mat zi, arma::colvec alpha_mu0,
                    arma::colvec alpha_mu, arma::mat matDw,  arma::mat matDw_u, arma::mat matDu,
                    arma::mat matB, arma::mat Sig, arma::mat G_mat_A_0_to_tau_i,
                    arma::mat G_mat_prod_A_0_to_tau,  double DeltaT){
...
//' 
// [[Rcpp::export]]
arma::mat MatCovPosterior(int K, int nD, arma::vec& mapping, arma::vec& paraOpt, arma::vec& paraFixe, arma::vec& posfix, arma::vec& m_is,
                          arma::mat& Mod_MatrixY, arma::vec& df, arma::mat& x,
                          arma::mat& z, arma::vec& q, int nb_paraD, arma::mat& x0, arma::mat& z0,
                          arma::vec& q0, arma::vec if_link, arma::vec& tau, arma::vec& tau_is, arma::mat& modA_mat, double DeltaT, int ntimes){
  
  
  mat matrixP = zeros<mat> (K,nD);
  // mat matcovres = zeros<mat> (ntimes*nD + 1 ,ntimes*nD);
  mat matcovres = zeros<mat> (ntimes*nD,ntimes*nD);
  ...
  // Y, model.matrix and transformation parameters
  Mat<double> Ytild = zeros<mat>(Mod_MatrixY.n_rows,K);
  ...
  for(int n=0; n<1;n++ ){
    ...
    mat G_mat_prod_A_0_to_tau = GmatprodAstotau(nD, vec_alpha_ij, tau, 0, DeltaT, modA_mat(span(n*m,((n+1)*m-1)), span(0,(L-1))));
    mat G_mat_A_0_to_tau_i = GmatA0totaui(nD, vec_alpha_ij, tau, DeltaT, modA_mat(span(n*m,((n+1)*m-1)), span(0,(L-1))));
    matcovres = MatCov(K, nD, matrixP, m_is(n), tau, tau_is(span(p,(p+m_is(n)-1))), Ytild(span(p,(p+m_is(n)-1)), span(0,(K-1))),
                       x0(span(n*nD,(n+1)*nD-1), span(0,(ncol_x0-1))),
                       z0(span(n*nD,(n+1)*nD-1), span(0,(ncol_z0-1))), x(span(n*nD*m,((n+1)*nD*m-1)), span(0,(ncol_x-1))),
                       z(span(n*nD*m,((n+1)*nD*m-1)), span(0,(ncol_z-1))),alpha_mu0, alpha_mu, matDw, matDw_u, matDu,
                       matB, Sig, G_mat_A_0_to_tau_i, G_mat_prod_A_0_to_tau,  DeltaT);
...
//' 
// [[Rcpp::export]]
arma::vec EsperancePosterior(int K, int nD, arma::vec& mapping, arma::vec& paraOpt, arma::vec& paraFixe, arma::vec& posfix, arma::vec& m_is,
                             arma::mat& Mod_MatrixY, arma::vec& df, arma::mat& x,
                             arma::mat& z, arma::vec& q, int nb_paraD, arma::mat& x0, arma::mat& z0,
                             arma::vec& q0, arma::vec if_link, arma::vec& tau, arma::vec& tau_is, arma::mat& modA_mat, double DeltaT, int ntimes){
  ...
  // mat matcovres = zeros<mat> (ntimes*nD + 1 ,ntimes*nD);
  mat matcovres = zeros<mat> (ntimes*nD,ntimes*nD);
  arma::vec esp = zeros(ntimes*nD,1); 
  ...
  for(int n=0; n<1;n++ ){
    ...
    mat G_mat_prod_A_0_to_tau = GmatprodAstotau(nD, vec_alpha_ij, tau, 0, DeltaT, modA_mat(span(n*m,((n+1)*m-1)), span(0,(L-1))));
    mat G_mat_A_0_to_tau_i = GmatA0totaui(nD, vec_alpha_ij, tau, DeltaT, modA_mat(span(n*m,((n+1)*m-1)), span(0,(L-1))));
    esp = Esperance(K, nD, matrixP, m_is(n), tau, tau_is(span(p,(p+m_is(n)-1))), Ytild(span(p,(p+m_is(n)-1)), span(0,(K-1))),
                    x0(span(n*nD,(n+1)*nD-1), span(0,(ncol_x0-1))),
                    z0(span(n*nD,(n+1)*nD-1), span(0,(ncol_z0-1))), x(span(n*nD*m,((n+1)*nD*m-1)), span(0,(ncol_x-1))),
                    z(span(n*nD*m,((n+1)*nD*m-1)), span(0,(ncol_z-1))),alpha_mu0, alpha_mu, matDw, matDw_u, matDu,
                    matB, Sig, G_mat_A_0_to_tau_i, G_mat_prod_A_0_to_tau,  DeltaT);
